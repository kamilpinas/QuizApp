  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        {lastquestion > qnumber
          ? RenderQuestion(navigation, quizContent, qnumber)
          : RenderFinalScore(
              navigation,
              quizContent.name,
              quizContent.tasks.length,
            )}
      </View>
    </SafeAreaView>
  );
    ...
    <View style={[{flex: 1}]}>
      <View
        ...
        <Text style={[{flex: 1, marginStart: 30}]}>
          Question {qnumber + 1} of {quizContent.tasks.length}
        </Text>
      ...
      <View
        style={[
          {flex: 1, justifyContent: 'center', marginStart: 40, marginEnd: 70},
        ]}>
        <Text style={{alignSelf: 'center'}}>
          {quizContent.tasks[qnumber].question}
        </Text>
      </View>
      <View style={{flex: 3}}>
  ...
  return (
    <View style={[{flex: 1, alignItems: 'center'}]}>
      ...
      <Text style={[{marginTop: 4}]}>
        {'Możliwa liczba punktow: ' + numberOfQuestions}
      </Text>
      <Text style={[{marginTop: 4}]}>
        {'Data: ' + new Date().toISOString().slice(0, 10)}
      </Text>
      <View style={[{marginTop: 10}]}>
        <Button title={'ranking'} onPress={() => navigation.navigate('Rank')} />
      </View>
    </View>
  );
}
