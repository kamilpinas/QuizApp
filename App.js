...
const getDB = () =>
  DB
    ? DB
    : (DB = SQLite.openDatabase({name: 'base.db', createFromLocation: 1}));
const querysToCreate = [
              ...
              NetInfo.fetch().then((state) => {
                if (state.isConnected == true) {
                  this.goToQuizOnline(
                    navigation,
                    allTestsData[
                      Math.floor(Math.random() * allTestsData.length)
                    ],
                  );
                } else {
                  this.goToQuizOffline(
                    navigation,
                    allTestsData[
                      Math.floor(Math.random() * allTestsData.length)
                    ],
  ...
  async getTestsDetails(db) {
    ...
    db.transaction((tx) => {
      ...
      tests.forEach((itm, i) => {
        let tasks = [];
        tx.executeSql(
          'SELECT * FROM questions WHERE id LIKE "' + itm.id + '" ;',
          [],
          (tx, resultsQuest) => {
            for (let j = 0; j < resultsQuest.rows.length; j++) {
              let answers = [];
              tx.executeSql(
                'SELECT * FROM answers WHERE question LIKE "' +
                  resultsQuest.rows.item(j).question +
                  '" ;',
                [],
                (tx, resultsAnswer) => {
      ...
      <NavigationContainer>
        <Drawer.Navigator
          ...
          <Drawer.Screen
            ...
            options={{title: 'Regulamin'}}
          />
          <Drawer.Screen
            name="Home"
            component={HomeScreen}
            options={{title: 'Strona glowna'}}
          />
          <Drawer.Screen
            name="Rank"
